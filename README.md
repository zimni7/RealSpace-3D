...
🧭 파이프라인 단계 설명
데이터 취득: StrayScanner 앱을 통해 공간을 스캔하고 PC로 가져온다.

전처리 및 시각화: 원본 포인트 클라우드를 3D로 시각화하여 데이터 상태를 확인한다.

구조 감지 (Detection): 벽, 바닥, 천장 평면을 감지하고 정보를 추출한다.

재구성 (Reconstruction): 추출된 정보를 바탕으로 노이즈 없는 직각 3D 모델을 생성한다.

텍스처 복원 (Texturing): 생성된 모델에 실제 영상의 텍스처를 입히고 빈 곳을 복원한다.
